...
add_filter( 'render_block', 'exs_filter_woocommerce_the_content', 10, 2 );
if ( ! function_exists( 'exs_filter_woocommerce_the_content' ) ) :
	function exs_filter_woocommerce_the_content( $html, $block ) {
		...
		$html =  str_replace( 'ul class="wc-block-grid__products"', 'ul class="products wc-block-grid__products"', $html );
		...
		if ( 'woocommerce/product-template' === $block['blockName'] ) {
			...
			$btn_block = exs_option( 'product_simple_add_to_cart_block_button', '' ) ? 'block-btn' : '';
			$center    = exs_option( 'product_center_content', '' ) ? 'text-center' : '';
			$html =  str_replace( 'wp-block-button ', ' wp-block-button ' . esc_attr( $btn_block . ' ' . $hide_icon . ' ' . $center ), $html );

			//ul
			$html =  str_replace( ' wc-block-product-template ', ' products wc-block-grid__products  wc-block-product-template ', $html );
			$html =  str_replace( ' columns-4 ', ' columns-4 wc-block-grid has-4-columns ', $html );
			$html =  str_replace( ' columns-3 ', ' columns-3 wc-block-grid has-3-columns ', $html );
			$html =  str_replace( ' columns-2 ', ' columns-2 wc-block-grid has-2-columns ', $html );
			//li
			$html = str_replace( 'class="wc-block-product ', 'class="wc-block-product wc-block-grid__product ', $html );
			//wrap in the product-loop-item
			$html = preg_replace( '/<li (.*)>/i', '<li ${1}><div class="product-loop-item">', $html );
